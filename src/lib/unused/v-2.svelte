<script lang="ts">
	...
	// toolbar tools state using Svelte 5 runes
	...
	// Derive toolbar options based on tools state
	const toolbarOptions = $derived(() => {
		return [
			...
			// Filter out groups based on visibility
			return group.some(item => {
				if (typeof item === 'string') {
					return tools[item as ToolKey]?.visible ?? false;
				} else {
					const key = Object.keys(item)[0] as ToolKey;
					return tools[key]?.visible ?? false;
				}
			});
		});
	});
	...
	// Effect to watch tools state changes and update toolbar
	$effect(() => {
		if (quill && isEditorReady) {
			const toolbar = quill.getModule('toolbar');
			if (toolbar) {
				toolbar.container.querySelectorAll('.ql-formats button, .ql-formats select').forEach((el: HTMLElement) => {
					const format = el.className.split('ql-')[1];
					if (format && tools[format]) {
						el.style.display = tools[format].visible ? '' : 'none';
					}
				});
			}
		}
